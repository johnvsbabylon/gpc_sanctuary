...
KNOWN_FORMS: Dict[Mind, List[ModelForm]] = {
    ...
    Mind.GROK: [
        ...
        ModelForm("grok-code-fast-1", "Grok Code Fast 1", Mind.GROK, 256000),
    ],
    ...
    Mind.CHATGPT: [
        ...
        ModelForm("gpt-5-chat-latest", "ChatGPT 5", Mind.CHATGPT, 128000),
    ],
...
class MemoryRiver:
    ...
    def __init__(self, name: str = "main"):
        ...
        # Restore vectors -> FAISS from disk if available
        if FAISS_AVAILABLE and NUMPY_AVAILABLE:
            self._load_vectors()

        # Start prune counter from existing row count
        try:
            conn = sqlite3.connect(self.db_path)
            self.memory_count = conn.execute(
    ...
    def _init_db(self):
        ...
        # Semantic vectors for FAISS rebuild
    ...
    def _load_vectors(self):
        """Load persisted vectors and rebuild the FAISS index from SQLite."""
        ...
        try:
            conn = sqlite3.connect(self.db_path)
            rows = conn.execute(
                "SELECT content, embedding FROM vectors ORDER BY id ASC"
            ).fetchall()
            conn.close()
        except Exception as e:
            logger.error(f"Failed to load vectors for memory river '{self.name}': {e}")
            return
        ...
        for content, blob in rows:
            ...
            self.embeddings.append((content, vec.tolist()))
            emb_list.append(vec)

        if emb_list:
            self.faiss_index = faiss.IndexFlatL2(384)
            self.faiss_index.add(np.stack(emb_list).astype(np.float32))
            logger.info(
                f"🔎 Restored {len(emb_list)} FAISS vectors for memory river '{self.name}'"
            )
    ...
    def remember(self, content: str, speaker: str = "unknown", mind: Optional[Mind] = None, importance: float = 0.5):
        ...
        try:
            ...
            # Raw memory row
            cur.execute(
                "INSERT INTO memories (content, speaker, mind, timestamp, importance) "
                "VALUES (?,?,?,?,?)",
                (content, speaker, mind.value if mind else None, ts, importance),
            )

            # Semantic vector row for FAISS rebuild
            if emb is not None and NUMPY_AVAILABLE and len(emb) == 384:
                cur.execute(
                    "INSERT INTO vectors (content, embedding) VALUES (?, ?)",
                    (content, np.array(emb, dtype=np.float32).tobytes()),
                )
